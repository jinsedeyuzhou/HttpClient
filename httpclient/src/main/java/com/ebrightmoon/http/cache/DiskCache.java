...
import com.ebrightmoon.http.util.AppUtil;
...
public class DiskCache implements ICache {
    ...
    private long cacheTime = HttpUtils.CACHE_NEVER_EXPIRE;

    public DiskCache(Context context) {
        this(context, getDiskCacheDir(context, HttpUtils.CACHE_DISK_DIR),
                calculateDiskCacheSize(getDiskCacheDir(context, HttpUtils.CACHE_DISK_DIR)));
    }
            ...
            if (snapshot != null) {
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    if ((createTime + expireMills > Calendar.getInstance().getTimeInMillis())
                            || expireMills == HttpUtils.CACHE_NEVER_EXPIRE) {
