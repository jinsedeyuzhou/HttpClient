...
public class DiskCache implements ICache {
    ...
    private long cacheTime = AppConfig.CACHE_NEVER_EXPIRE;

    public DiskCache(Context context) {
        this(context, getDiskCacheDir(context, AppConfig.CACHE_DISK_DIR),
                calculateDiskCacheSize(getDiskCacheDir(context, AppConfig.CACHE_DISK_DIR)));
    }
            ...
            if (snapshot != null) {
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    if ((createTime + expireMills > Calendar.getInstance().getTimeInMillis())
                            || expireMills == AppConfig.CACHE_NEVER_EXPIRE) {
