...
public abstract class BaseHttpRequest<R extends BaseHttpRequest> extends BaseRequest<R> {
    ...
    protected void generateLocalConfig() {
        ...
        if (isLocalCache) {
            if (cacheKey != null) {
                HttpClient.getApiCacheBuilder().cacheKey(cacheKey);
            } else {
                HttpClient.getApiCacheBuilder().cacheKey(ApiHost.getHost());
            }
            if (cacheTime > 0) {
                HttpClient.getApiCacheBuilder().cacheTime(cacheTime);
            } else {
                HttpClient.getApiCacheBuilder().cacheTime(AppConfig.CACHE_NEVER_EXPIRE);
            }
        }
        if (baseUrl != null && isLocalCache && cacheKey == null) {
            HttpClient.getApiCacheBuilder().cacheKey(baseUrl);
        }
